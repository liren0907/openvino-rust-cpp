...
void Tracker::SolveAssignmentProblem(
    ...
    auto solved_pairs = KuhnMunkres().Solve(dissimilarity);
    ...
    for (size_t id : track_ids) {
        if (i < solved_pairs.size()) {
            size_t row_idx = solved_pairs[i].first;
            size_t col_idx = solved_pairs[i].second;

            if (col_idx < detections.size()) {
                matches->emplace(id, col_idx, 1 - dissimilarity.at<float>(row_idx, col_idx));
            } else {
