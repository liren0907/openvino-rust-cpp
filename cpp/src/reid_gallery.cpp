...
EmbeddingsGallery::EmbeddingsGallery(const std::string& ids_list,
    ...
    cv::FileNode identities_node = fs["identities"];
    CV_Assert(identities_node.type() == cv::FileNode::SEQ);

    int id = 0;
    for (size_t idx = 0; idx < identities_node.size(); idx++) {
        cv::FileNode identity_item = identities_node[idx];
        std::string label = identity_item["label"].string();
        cv::FileNode images_node = identity_item["images"];

        CV_Assert(images_node.size() == 1);

        std::vector<cv::Mat> embeddings;
        for (size_t i = 0; i < images_node.size(); i++) {
            std::string path;
            if (file_exists(images_node[i].string())) {
                path = images_node[i].string();
            } else {
                path = folder_name(ids_list) + separator() + images_node[i].string();
            }

            cv::Mat image = cv::imread(path);
            if (image.empty()) {
                slog::warn << "Failed to load image: " << path << slog::endl;
                continue;
            }
            cv::Mat emb;
            ...
            if (status == RegistrationStatus::SUCCESS) {
                ...
                slog::warn << "Failed to register identity: " << label << " (image: " << path << ")" << slog::endl;
            }
        }
    }
    slog::info << "Loaded " << id << " identities from gallery" << slog::endl;
}
...
std::vector<int> EmbeddingsGallery::GetIDsByEmbeddings(const std::vector<cv::Mat>& embeddings) const {
    ...
    for (const auto& match_pair : matched_idx) {
        size_t row_idx = match_pair.first;
        size_t col_idx = match_pair.second;

        if (static_cast<int>(col_idx) == -1) {
            output_ids.push_back(unknown_id);
            continue;
        }

        if (distances.at<float>(row_idx, col_idx) > reid_threshold)
            output_ids.push_back(unknown_id);
        else
