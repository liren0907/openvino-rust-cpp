...
pub struct Video {
    pub(crate) inner: cxx::UniquePtr<ovi_vision_sys::FrameDataWrapper>,
}
...
pub struct Frame<'a> {
    pub(crate) inner: cxx::UniquePtr<ovi_vision_sys::FrameRef>,
...
impl Video {
    ...
    pub fn open(path: &str) -> Result<Self> {
        let inner = ovi_vision_sys::open_video(path, false, 0)?;
        Ok(Self { inner })
    }
    ...
    pub fn open_with_options(path: &str, loop_video: bool, read_limit: u32) -> Result<Self> {
        let inner = ovi_vision_sys::open_video(path, loop_video, read_limit)?;
        Ok(Self { inner })
    }
    ...
    pub fn read(&mut self) -> Result<bool> {
        Ok(ovi_vision_sys::read_frame(self.inner.pin_mut())?)
    }
    ...
    pub fn current_frame(&self) -> Result<Frame<'_>> {
        let inner = ovi_vision_sys::current_frame(&self.inner)?;
    ...
    pub fn width(&self) -> i32 {
        ovi_vision_sys::frame_width(&self.inner)
    }

    /// Height of the current frame.
    pub fn height(&self) -> i32 {
        ovi_vision_sys::frame_height(&self.inner)
    }

    /// Video FPS.
    pub fn fps(&self) -> f64 {
        ovi_vision_sys::video_fps(&self.inner)
    }
}
