...
DEFINE_string(i, "", "Required. Path to input video file or image.");
DEFINE_string(o, "", "Optional. Name of the output video file.");

// OpenVINO runtime arguments
DEFINE_string(d_act, "CPU", "Optional. Target device for Person/Action Detection.");
DEFINE_string(d_fd, "CPU", "Optional. Target device for Face Detection.");
DEFINE_string(d_lm, "CPU", "Optional. Target device for Facial Landmarks Regression.");
DEFINE_string(d_reid, "CPU", "Optional. Target device for Face Reidentification.");
DEFINE_string(m_act, "", "Optional. Path to the Person/Action Detection model (.xml).");
DEFINE_string(m_fd, "", "Optional. Path to the Face Detection model (.xml).");
DEFINE_string(m_lm, "", "Optional. Path to the Facial Landmarks Regression model (.xml).");
DEFINE_string(m_reid, "", "Optional. Path to the Face Reidentification model (.xml).");
DEFINE_bool(greedy_reid_matching, false, "Optional. Use faster greedy matching in face reid.");

// Face detector arguments
DEFINE_uint32(inh_fd, 600, "Optional. Face detector input height.");
DEFINE_uint32(inw_fd, 600, "Optional. Face detector input width.");
DEFINE_double(t_fd, 0.6, "Optional. Face detector threshold.");
DEFINE_double(t_reg_fd, 0.6, "Optional. Face detector threshold for registry.");
DEFINE_double(exp_r_fd, 1.15, "Optional. Face detector bbox expansion coefficient.");

// Action detector arguments
DEFINE_double(t_ad, 0.3, "Optional. Action detector threshold.");
DEFINE_double(t_ar, 0.75, "Optional. Action recognition threshold.");

// Face reid arguments
DEFINE_double(t_reid, 0.7, "Optional. Face reid threshold.");
DEFINE_uint32(min_size_fr, 128, "Optional. Minimum input size for faces during reid.");
DEFINE_bool(crop_gallery, false, "Optional. Crop gallery images.");
DEFINE_string(fg, "", "Optional. Path to the face gallery file.");
DEFINE_string(teacher_id, "", "Optional. ID of a teacher.");
DEFINE_string(student_ac, "sitting,standing,raising_hand", "Optional. Student actions.");
DEFINE_string(top_ac, "sitting,writing,raising_hand,standing,turned_around,lie_on_the_desk", "Optional. Top-view student actions.");
DEFINE_string(teacher_ac, "standing,writing,demonstrating", "Optional. Teacher actions.");

// Other arguments
DEFINE_bool(loop, false, "Optional. Loop the input video.");
DEFINE_bool(no_show, false, "Optional. Don't show output window.");
DEFINE_uint32(read_limit, 0, "Optional. Read only specified number of frames.");
DEFINE_bool(h, false, "Optional. Show help message.");
DEFINE_string(u, "", "Optional. FURL of CVAT UI.");
DEFINE_int32(limit, 0, "Optional. Number of frames to store in output.");

// Detection arguments
DEFINE_int32(a_top, 0, "Optional. Number of top students.");
DEFINE_string(top_id, "raising_hand", "Optional. Target action for top students.");
DEFINE_string(ad, "", "Optional. Path to action detections JSON.");
DEFINE_string(al, "", "Optional. Path to action recognitions JSON.");
DEFINE_string(r, "", "Optional. Path to face identities JSON.");
DEFINE_double(d_ad, 0.5, "Optional. Action duration in seconds.");
DEFINE_double(min_ad, 0.5, "Optional. Minimum action duration after merging.");
DEFINE_int32(ss_t, -1, "Optional. Number of frames to smooth actions.");
...
static void showUsage() {
    std::cout << "\nsmart_classroom_example [OPTION]\n"
              << "Options:\n\n"
              << "    -h                         Print help\n"
              << "    -i                         Required. Path to input video/image\n"
              << "    -o                         Optional. Output video file\n"
              << "    -m_act                     Person/Action Detection model\n"
              << "    -m_fd                      Face Detection model\n"
              << "    -m_lm                      Facial Landmarks model\n"
              << "    -m_reid                    Face Reidentification model\n"
              << "    -fg                        Face gallery JSON\n"
              << "    -no_show                   Don't show output\n"
              << std::endl;
}

// ========== Application logic (moved from demos/smart_classroom/main.cpp) ==========
...
public:
    ...
    void Show() {
        if (m_enabled) cv::imshow(m_main_window_name, m_frame);
        m_writer.write(m_frame);
    }

    void DrawCrop(cv::Rect roi, int id, const cv::Scalar& color) const {
        if (!m_enabled || m_num_top_persons <= 0) return;
        if (id < 0 || id >= m_num_top_persons) return;
    ...
    void Finalize() const {
        if (m_enabled) {
            ...
            if (m_num_top_persons > 0) cv::destroyWindow(m_top_window_name);
        }
    }
};
...
void SmoothTracks(const std::map<int, FrameEventsTrack>& obj_id_to_actions_track,
    ...
    for (const auto& tup : obj_id_to_actions_track) {
        ...
        if (frame_events.empty()) continue;
        ...
        for (size_t frame_id = 1; frame_id < frame_events.size(); ++frame_id) {
            ...
            if (last_range_event.end_frame_id + window_size - 1 >= cur_frame_event.frame_id &&
                ...
                range_events.emplace_back(cur_frame_event.frame_id, cur_frame_event.frame_id + 1, cur_frame_event.action);
            }
        }
        if (range_events.back().end_frame_id - range_events.back().begin_frame_id < min_length) {
            range_events.pop_back();
...
std::string GetActionTextLabel(const unsigned label, const std::vector<std::string>& actions_map) {
    if (label < actions_map.size()) return actions_map[label];
...
cv::Scalar GetActionTextColor(const unsigned label) {
    ...
    if (label < arraySize(label_colors)) return label_colors[label];
...
public:
    ...
    std::string GetLabelByID(int) const override { return EmbeddingsGallery::unknown_label; }
...
public:
    FaceRecognizerDefault(
            ...
            double reid_threshold, int min_size_fr,
            bool crop_gallery, bool greedy_reid_matching) :
        ...
        face_gallery(face_gallery_path, reid_threshold, min_size_fr, crop_gallery,
                     face_registration_det_config, landmarks_detector, face_reid, greedy_reid_matching) {
    ...
    bool LabelExists(const std::string& label) const override { return face_gallery.LabelExists(label); }
    std::string GetLabelByID(int id) const override { return face_gallery.GetLabelByID(id); }
    std::vector<std::string> GetIDToLabelMap() const override { return face_gallery.GetIDToLabelMap(); }
    ...
    std::vector<int> Recognize(const cv::Mat& frame, const detection::DetectedObjects& faces) override {
        ...
        std::vector<cv::Mat> landmarks, embeddings, face_rois;
        auto face_roi = [&](const detection::DetectedObject& face) { return frame(face.rect); };
...
int main(int argc, char* argv[]) {
    try {
        ...
        const auto actions_type = FLAGS_teacher_id.empty() ?
            (FLAGS_a_top > 0 ? TOP_K : STUDENT) : TEACHER;
        if (!FLAGS_ad.empty()) {
        ...
        const auto actions_map = actions_type == STUDENT ?
            ...
            split(FLAGS_top_ac, ',') : split(FLAGS_teacher_ac, ',');
        ...
        LazyVideoWriter videoWriter{FLAGS_o, cap->fps(), static_cast<size_t>(FLAGS_limit)};
        Visualizer sc_visualizer(!FLAGS_no_show, videoWriter, num_top_persons);
        DetectionsLogger logger(slog::debug, !FLAGS_r.empty(), FLAGS_ad, FLAGS_al);
        ...
        while (!is_last_frame) {
            ...
            if (actions_type == TOP_K) {
                    ...
                    if (static_cast<int>(top_k_obj_ids.size()) < FLAGS_a_top) {
                        for (const auto& action : tracked_actions) {
                            if (action.label == top_action_id && top_k_obj_ids.count(action.object_id) == 0) {
                                ...
                                if (static_cast<int>(top_k_obj_ids.size()) >= FLAGS_a_top) break;
                            }
                        }
                    }
                    for (const auto& action : tracked_actions) {
                ...
                for (size_t j = 0; j < tracked_faces.size(); j++) {
                    ...
                    if (person_ind >= 0) action_ind = tracked_actions[person_ind].label;
        ...
        if (actions_type == STUDENT) {
            ...
            if (!face_id_to_label_map.empty()) {
                ...
                ConvertActionMapsToFrameEventTracks(face_obj_id_to_action_maps, default_action_index, &face_obj_id_to_actions_track);
                ...
                SmoothTracks(face_obj_id_to_actions_track, start_frame, end_frame,
                             smooth_window_size, smooth_min_length, default_action_index, &face_obj_id_to_events);
                logger.DumpTracks(face_obj_id_to_events, actions_map, face_track_id_to_label, face_id_to_label_map);
                std::vector<std::map<int, int>> face_obj_id_to_smoothed_action_maps;
                ConvertRangeEventsTracksToActionMaps(end_frame, face_obj_id_to_events, &face_obj_id_to_smoothed_action_maps);
                logger.DumpDetections(FLAGS_i, frame.size(), work_num_frames, new_face_tracks,
                                      face_track_id_to_label, actions_map, face_id_to_label_map, face_obj_id_to_smoothed_action_maps);
            }
        }
